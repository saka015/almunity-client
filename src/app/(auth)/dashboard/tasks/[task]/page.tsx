    ...
    <main className="min-h-screen p-6 text-teal-900">
      ...
      <div className="mt-8 flex flex-col md:flex-row gap-3 w-full md:min-h-[600px]">
        <div className="w-full md:w-2/3 border-b mb-4 md:border-b-0 md:border-r border-teal-600 pr-0 md:pr-4">
          <pre className="whitespace-pre-wrap text-teal-200">{task.description}</pre>
        </div>
        <div className="w-full md:w-1/3 md:pl-4">
          <div className="flex pb-4 text-teal-400">
            Price <span className="font-semibold ml-3 text-teal-600">₹{task?.price}</span>
          </div>

          {task.createdBy === currentUser?._id ? (
            <div className="mt-1">
              <h1 className="text-teal-400 text-xl mb-2">Applicants</h1>
              {task?.applicants?.length === 0 && (
                <p className="text-teal-300 ">No applicants yet!</p>
              )}
              {task?.applicants?.map((applicant: any, index: number) => (
                <div
                  key={index}
                  className="text-teal-300 py-1 border-b border-teal-700 last:border-b-0"
                >
            ...
            <form onSubmit={handleApply} className="grid gap-4 py-4">
              <h2 className="text-xl text-teal-300 mb-2">Apply for this Task</h2>
              <div className="flex flex-col gap-4">
                <Input
                  id="title"
                  type="text"
                  placeholder="Portfolio/GitHub link (optional)"
                  className="col-span-3 bg-teal-700 text-white border-gray-600 placeholder-teal-400"
              ...
              <Button type="submit" className="bg-teal-800 hover:bg-teal-700" disabled={isApplying}>
