    ...
    <main className="min-h-screen p-6 text-slate-900">
      ...
      <div className="mt-8 flex flex-col md:flex-row gap-3 w-full md:min-h-[600px]">
        <div className="w-full md:w-2/3 border-b mb-4 md:border-b-0 md:border-r border-slate-600 pr-0 md:pr-4">
          <pre className="whitespace-pre-wrap text-slate-200">{product.description}</pre>
        </div>
        <div className="w-full md:w-1/3 md:pl-4">
          <div className="flex pb-4 text-slate-400">
            Price <span className="font-semibold ml-3 text-cyan-600">₹{product?.price}</span>
          </div>
          {product.productType === 'session' && (
            <div className="flex pb-4 text-slate-400">
              Duration{' '}
              <span className="font-semibold ml-3 text-cyan-600">{product?.duration} mins</span>
            </div>
          )}

          <div className="flex pb-4 text-slate-400">
            Type{' '}
            <span className="font-semibold ml-3 text-cyan-600 capitalize">
              {product?.productType}
            </span>
          </div>
          ...
          {product.createdBy._id === currentUser?._id ? (
            <div className="mt-1">
              <h1 className="text-slate-400 text-xl mb-2">Booking Details</h1>
              {!product.bookedBy ? (
                <p className="text-slate-300">Not booked yet!</p>
              ) : (
                <div className="text-slate-300 py-1 border-b border-slate-700 last:border-b-0">
